...
def update_product():
    ...
    if product_item is None:
        ...
        return
    choice = input(
        "What whould you like to change : 1)product name 2)category 3)product price 4)product inventory 5)product supplier 6)product offer")
    if choice == '1':
...
def list_products():
    ...
    choice = input(
        "Please choose what whould you like to see : 1)All products 2)Products with offer 3)Category 4)Name ").lower()
    if choice == "1":
        i = 1
        for product in products:
        ...
        for product in products:
            if product.product_id == product_id:
                if product.inventory < quantity:
                    print(
                        f"Insufficient inventory for product '{product.name}' (Product ID: {product_id}). Order not executed.")
                else:
        ...
        with open(file_name, "w") as file:
            ...
            for user in Admins + Shoppers:
                ...
                if isinstance(user, Shopper):
                    ...
                    if user.basket:
                        basket_data = ';'.join(
                            [f"{product_id}:{quantity}" for product_id, quantity in user.basket.items()])
